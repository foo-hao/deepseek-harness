...
Process shutdown gives the plugin tree up to five seconds to dispose. In an interactive terminal, the first `SIGINT` (`Ctrl+C`) does not exit — it prints a prompt and arms a three-second confirmation window, and only a second `Ctrl+C` inside that window begins the graceful drain (which reports 130). The window expiring cancels the exit. `SIGTERM` is a supervisor's ordinary stop request and always bypasses confirmation, exiting 0 on every surface; non-interactive (non-TTY) processes keep the single-signal behavior. During an active drain, a further `SIGINT` forces immediate exit, and if one-shot normal completion is already stuck in disposal the first `Ctrl+C` is the escalation and exits immediately instead of being swallowed.

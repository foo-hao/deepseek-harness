...
进程关闭时，插件树最多有 5 秒完成 dispose。在交互式终端中，首次收到 `SIGINT`（`Ctrl+C`）不会退出：它会打印提示并启动一个 3 秒的确认窗口，只有在该窗口内再次按下 `Ctrl+C` 才会开始优雅排空（并报告 130）；窗口超时则取消退出。`SIGTERM` 是监督进程发出的常规停止请求，始终绕过确认，在所有运行模式下都以 0 退出；非交互（非 TTY）进程保持单次信号即退出的行为。在排空进行中再次收到 `SIGINT` 会立即强制退出；如果一次性运行在正常结束时已经卡在 dispose 阶段，第一次按下 `Ctrl+C` 就会直接升级为强制退出，而不会被忽略。

...
- `suggest(session, signal?)` generates on-demand candidate titles from the whole session without committing: it runs the provider's optional `suggest` (falling back to a single `generate`), then normalizes and deduplicates the titles. It never appends a `session/title` event, never pins the title, and never disturbs the automatic state machine; an empty result means no provider is mounted or no eligible text exists.

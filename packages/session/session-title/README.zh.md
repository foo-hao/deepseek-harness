...
- `suggest(session, signal?)` 根据整个会话按需生成候选标题且不提交：调用提供方可选的 `suggest`（缺失时回退为单次 `generate`），然后规范化并去重。该操作不追加 `session/title` 事件、不钉住标题，也不干扰自动状态机；空结果表示未挂载提供方或不存在符合条件的文本。
...
提供方会提供带品牌类型的稳定 id、自动模式（`first-prompt` 或 `all-prompts`）和 `generate(request)`；可选的 `suggest(request)` 为按需重命名建议界面返回多个候选项。请求携带活跃会话、截至一次固定修订的所有符合条件消息、可用时当前已记录的主请求路由，以及取消信号。结果包含非空标题、该请求中互不重复且有序的来源消息 seq，以及生成该标题时使用的可选提供方／模型路由。服务会在结果持久保存前进行规范化和验证。

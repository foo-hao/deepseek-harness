...
export class SessionManager {
   ...
   * @param opts - source session, the optional seq anchoring the cut, and the
   *   optional target cwd relocating the child into another project.
   * @returns the fork result (the child session id).
   */
  async fork(
    opts: { sessionId: SessionId; atSeq?: number; cwd?: string },
  ): Promise<RpcResult<{ sessionId: SessionId }>> {
    try {

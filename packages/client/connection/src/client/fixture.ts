  ...
  const api: ApiProxy = {
    sessions: {
      ...
      suggestTitles: (request) => {
        const missing = requireSession(request)
        if (missing !== undefined) return missing
        return ok(request, { titles: ['Refactor session title', 'Summarize conversation goal', 'Rename to current task'] })
      },
      fork: (request) => {
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'session.suggestTitles': return this.api.sessions.suggestTitles(request, signal)

...
describe('session rename through the assembled browser', () => {
  ...
  it('shows AI title suggestions and clicking one prefills the rename input', async () => {
    const runtime = await createRuntime()
    const rename = vi.fn<ISession['rename']>(async title => ({
      ok: true, value: { title, seq: 7 },
    }))
    const suggestTitles = vi.fn<ISession['suggestTitles']>(async () => ({
      ok: true, value: { titles: ['总结分叉实验', '记录实验数据'] },
    }))
    await runtime.sessions.add({
      id: SID,
      summary: { title: '旧标题', displayTitle: '旧标题', cwd: '/w/alpha' },
      session: { rename, suggestTitles },
    })
    await runtime.workspaces.update((draft) => {
      draft.items = [{
        workspaceId: 'w1' as WorkspaceId, title: 'alpha', path: '/w/alpha',
    ...
    // Suggestions load asynchronously; clicking one prefills the draft.
    const chip = await view.findByRole('button', { name: '使用建议“总结分叉实验”' })
    fireEvent.click(chip)
    const input = view.getByLabelText('会话名称') as HTMLInputElement
    expect(input.value).toBe('总结分叉实验')

    // Confirming submits the suggested title through the rename hop.
    fireEvent.click(view.getByRole('button', { name: '重命名' }))
    await waitFor(() => { expect(rename).toHaveBeenCalledWith('总结分叉实验') })
    await runtime.dispose()
  })
})

...
describe('workspace browser rows', () => {
  it('omits only an empty leading status slot in the hierarchy-free flat list', () => {
    ...
    const view = render(<SessionNodeItem node={idle} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onMoveToProject={vi.fn()} onArchive={vi.fn()} flat t={t} />)
    ...
    view.rerender(<SessionNodeItem node={{ ...idle, running: true }} currentId={undefined} now={0}
      onOpen={vi.fn()} onRename={vi.fn()} onFork={vi.fn()} onMoveToProject={vi.fn()} onArchive={vi.fn()} flat t={t} />)
  ...
  it('renders and opens a selected running Session row', () => {
    ...
    render(
      <SessionNodeItem node={node} currentId={node.id} now={0} onOpen={onOpen}
        onRename={vi.fn()} onFork={vi.fn()} onMoveToProject={vi.fn()} onArchive={vi.fn()} t={t} />,
    )
  ...
  it('session row menu opens without opening the session and dispatches rename, fork, move, and archive', () => {
    const onOpen = vi.fn()
    const onRename = vi.fn()
    const onFork = vi.fn()
    ...
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={onOpen}
      onRename={onRename} onFork={onFork} onMoveToProject={onMoveToProject} onArchive={onArchive} t={t} />)
    ...
    // Move dispatches with the session id (the browser opens the picker dialog).
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '移到其他工程' }))
    expect(onMoveToProject).toHaveBeenCalledWith(node.id)
  ...
  it.each([
    ...
    try {
      ...
      view.rerender(<SessionNodeItem node={{ ...node, running: false }} currentId={undefined} now={0}
        onOpen={vi.fn()} onRename={vi.fn()} onFork={vi.fn()} onMoveToProject={vi.fn()} onArchive={vi.fn()} t={t} />)
  ...
  it('draggable row wires start/end and gates hover/drop on an active same-group drag', () => {
    ...
    const { rerender } = render(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onMoveToProject={vi.fn()} onArchive={vi.fn()} drag={inactive} t={t} />,
    )
    const row = screen.getByRole('treeitem')
    stubRect(row)
    ...
    rerender(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onMoveToProject={vi.fn()} onArchive={vi.fn()} drag={active} t={t} />,
    )
    ...
    rerender(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onMoveToProject={vi.fn()} onArchive={vi.fn()} drag={after} t={t} />,
    )

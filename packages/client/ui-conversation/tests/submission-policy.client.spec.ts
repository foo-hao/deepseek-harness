...
it('adopts and persists idle mode independently of busy mode', () => {
  const host = stubSettingsScope<ConversationSettings>()
  const policy = new ComposerSubmissionPolicy(host.scope)
  expect(policy.enterMode.getSnapshot()).toBe('send')
  policy.setEnterMode('newline')
  expect(host.set).toHaveBeenCalledExactlyOnceWith('enterMode', 'newline')
  policy.setEnterMode('newline')
  expect(host.set).toHaveBeenCalledTimes(1)
  host.publish({ status: 'ready', value: { busyEnter: 'queue', enterMode: 'send' }, revision: 1, writable: true })
  expect(policy.enterMode.getSnapshot()).toBe('send')
  expect(host.set).toHaveBeenCalledTimes(1)
  policy.setEnterMode('newline')
  expect(policy.resolve(true, 'enter', true)).toBe('queue')

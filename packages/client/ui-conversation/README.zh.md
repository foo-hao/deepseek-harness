...
通用设置分别提供空闲和繁忙状态的 Enter 偏好。空闲时 Enter 默认发送；选择换行后，普通 Enter 在编辑器中插入换行，Cmd/Ctrl+Enter 发送。Shift+Enter 插入换行。繁忙时 Enter 按排队或插话偏好提交，加速组合键使用另一种行为。菜单选择和输入法候选确认优先处理。空闲换行模式的占位提示保留当前操作说明并显示两种快捷键。两项偏好均通过 Host 设置服务持久化。
